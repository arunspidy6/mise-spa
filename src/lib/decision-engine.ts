...
function effortLevel(recipe: Recipe): ConfidenceLevel {
  // Low effort is the *good* outcome, so we invert difficulty/time.
  const t = recipe.time_minutes ?? 40;
  const d = recipe.difficulty ?? 2;
  // Effort tracks hands-on difficulty more than total time — a hands-off tray
  // bake that roasts for an hour is still low effort.
